...
class TestPoRModelCalculations:
    ...
    def test_self_por_score(self, E_base: float, delta: float, Q_self: float, expected: float):
        assert math.isclose(
            PoRModel.self_por_score(E_base, delta, Q_self),
            expected,
            rel_tol=1e-9
        )
    ...
    def test_semantic_gravity(self, freq: float, entropy: float, expected: float):
        assert math.isclose(
            PoRModel.semantic_gravity(freq, entropy),
            expected,
            rel_tol=1e-9
        )
    ...
    def test_por_collapse_frequency(self, lam: float, t: float, expected: float):
        ...
        assert math.isclose(
            PoRModel.por_collapse_frequency(lam, t),
            expected,
            rel_tol=1e-9
        )
    ...
    def test_refire_difference_type_and_value(self):
        # 正常ケース
    ...
    def test_self_coherence(self, ref_flow: float, d_in: float, d_out: float, expected: float):
        assert math.isclose(
            PoRModel.self_coherence(ref_flow, d_in, d_out),
            expected,
            rel_tol=1e-9
        )
        # ゼロ除算エラー
    ...
    def test_phase_gradient(self, k: float, E: float, S: float, gamma: float, expected: float):
        assert math.isclose(
            PoRModel.phase_gradient(E, S, k, gamma),
            expected,
            rel_tol=1e-9
        )
        # 負の S はエラー
